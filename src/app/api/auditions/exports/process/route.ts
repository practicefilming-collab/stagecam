...
import { buildAuditionTakeExportPlayback } from '@/lib/auditions/export-playback';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const playback = await buildAuditionTakeExportPlayback(claimed.audition_take_id);
    if (!playback) {
      throw new Error('Rehearsal playback data unavailable');
    }

    const render = await renderSceneExportToFile(playback, async (pct) => {
      await admin.from('audition_replay_export_jobs').update({ progress_pct: pct }).eq('id', claimed.id);
    });
    tmpRoot = render.tmpRoot;

    const key = `exports/auditions/${claimed.audition_take_id}/${claimed.id}.mp4`;
    await r2.send(
